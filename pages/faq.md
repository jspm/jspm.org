...
Import maps enable dynamic linking in the browser, something previously only complex bundlers can do in JavaScript, enabling easier dependency remapping in development and production environments.
...
Browsers can load 100s of modules in fractions of a second. The JSPM [online generator](https://generator.jspm.io) is a great example of a mid-sized application using these techniques.
...
Furthermore, what usually what causes slow JavaScript applications is shipping **unnnecessary code**. The techniques used in JSPM always ensure only necessary code is loaded, so that JSPM apps tend to be very fast.
